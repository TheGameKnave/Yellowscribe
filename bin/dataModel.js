...
class Unit {
    name;
    type; // unit (if it contains pieces) or game piece if it's a single model/figure/token
    uuid = require('crypto').randomBytes(4).toString("hex");
    meta = new Map(); // meta information about the unit
    unitClass; // whatever the classification of the unit is
    unitAsset; // the asset that was used to create this unit
    models = new Map(); // uuid -> model; each discrete model/figure/token in the unit.
    // A unit can be a single game piece, in which case it will have
    // no objects in models; the unitAsset will be the only asset.

    constructor(name, type, unitClass, unitAsset) {
        this.name = name;
        this.type = type;
    ...
    setUnitAsset(unitAsset) {
        this.unitAsset = unitAsset;
    }

    addmodel(model) {
        this.models.set(require('crypto').randomBytes(8).toString("hex"), model);
    }
...
class Asset {
    ...
    createDescription(unit) {
        this.ttsDescription = '';
        let assetClusters = this.findClusters(this.assets);
        // console.log(147,assetClusters);
        let nicknameAppend = [];
        ...
        assetClusters.forEach((assetCluster, k) => {
            if (assetCluster.length === 1 && (assetCluster[0].group || Object.keys(assetCluster[0].stats || {}).length || (assetClusters[k + 1] && assetCluster[0].assetDepth < assetClusters[k + 1][0].assetDepth))) this.ttsDescription += this.makeTtsDescription(assetCluster[0], initialDepth, assetCluster[0].assetDepth === initialDepth);
                ...
                assetCluster.forEach((asset, i) => {
                    // console.log(159, asset);
                    // console.log(160, asset.name);
                    let assetWidth = this.calculateStringLength(asset.name);
                    let spaceLeft = 70 - currWidth;
        ...
        if (this.meta?.ttsPartOfUnit) {
            let groupAssetClusters = this.findClusters(unit.unitAsset?.assets || [], true);
        ...
        this.ttsNickname += (unit.meta.ttsDamageStat && this.stats[unit.meta.ttsDamageStat] !== undefined ? '\n[00ff33]' + this.stats[unit.meta.ttsDamageStat] + '/' + this.stats[unit.meta.ttsDamageStat] + '[-] ' : '');
    }

    findClusters(assets, skipmodels = false) {
        let assetClusters = [];
        let curClust = [];
        let assetList = skipmodels ? this.filtermodelsAndSubAssets(assets) : assets;
    ...
    filtermodelsAndSubAssets(assets) {
    ...
    makeTtsDescription(asset, initialDepth, fancyStats = false) {
        ...
        if (fancyStats) descFrag = this.descriptionStats(asset, fancyStats);
            ...
            if (asset.group) {
                let groupColor = ['combat', 'attack', 'weapon', 'assault', 'strike', 'blast', 'siege', 'raid'].some(word => asset.group.toLowerCase().includes(word)) ? 'ff6666' : 'ccaaff';
    ...
    descriptionStats(asset, fancyStats = false, initialDepth = 0) {
        ...
        if (!fancyStats) {
            ...
            Object.keys(displayStats || {}).forEach((statName, i) => {
                ...
                // console.log(270, statName, statValue);
            ...
            Object.keys(displayStats || {}).forEach((statName, i) => {
                let statValue = displayStats[statName];
                // console.log(289, statName, statValue);
                let statNameWidth = this.calculateStringLength(statName);
                // console.log(291, statNameWidth);
...
module.exports = { Roster, Unit, Asset, AssetGroup }